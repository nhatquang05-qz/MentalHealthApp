...
import { StyleSheet, Text, View, TouchableOpacity, SafeAreaView, ScrollView, Modal, Linking } from 'react-native';
...
export default function SpecificResultScreen() {
  ...
  const getResultAnalysis = () => {
    const normalResult = currentTestData.results.find(r => score >= r.min && score <= r.max) 
    ...
    if (isCritical) {
      ...
      return {
        ...severeResult,
        description: "Mặc dù tổng điểm đánh giá của bạn không cao, nhưng câu trả lời về ý định làm hại bản thân là một dấu hiệu nghiêm trọng cần được chú ý đặc biệt.\n\nHãy tìm kiếm sự hỗ trợ ngay lập tức.",
  ...
  const renderMedicalContact = ({ item }: { item: MedicalContact }) => (
    <View style={[styles.contactItem, { backgroundColor: isDark ? '#333' : '#FFF5F5', borderColor: colors.danger }]}>
      <View style={{flex: 1}}>
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: isDark ? '#121212' : '#FFF9E1' }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          ...
          <Text style={[styles.resultDesc, { color: colors.text, fontWeight: isCritical ? '600' : '400' }]}>
          ...
          >
            <Text style={[styles.detailButtonText, { color: analysis.color }]}>📖 Xem lời khuyên chi tiết & Hỗ trợ</Text>
          </TouchableOpacity>
        ...
        <Text style={styles.disclaimerText}>*Kết quả chỉ mang tính chất tham khảo, không thay thế chẩn đoán y khoa.*</Text>
      </ScrollView>
        ...
        <View style={styles.modalContainer}>
          <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {(isCritical || (analysis.medicalContacts && analysis.medicalContacts.length > 0)) && (
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 24, padding: 30, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 5, alignItems: 'center', marginBottom: 30 },
  ...
  detailButton: { width: '100%', paddingVertical: 12, borderRadius: 30, alignItems: 'center', borderWidth: 1, marginBottom: 12, backgroundColor: 'transparent' },
  ...
  contactItem: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 12, borderWidth: 1, borderLeftWidth: 5 },
  contactName: { fontWeight: 'bold', fontSize: 15, marginBottom: 4 },
  contactAddress: { fontSize: 12 },
  callButton: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#34C759', justifyContent: 'center', alignItems: 'center', marginLeft: 10 },
  sourceText: { textAlign: 'center', fontSize: 12, marginTop: 20, fontStyle: 'italic', opacity: 0.6 },
